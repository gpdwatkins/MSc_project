...
# also want to it be able to play the game with differnt amounts of 'sight range'
...
#     board[cat_pos] = 'C'
#     board[mouse_pos] = 'M'
#     # only include the following line if we want to have the probability distribution
...
#         board[mouse_pos] = 'M'
#
#         if cat_can_see_mouse(cat_pos, mouse_pos, 2):
...
def play_cat_and_mouse(board_height, board_width, show_figs = True, policy = None, sight = float('inf'), mouse_pos_dist = False):
    ...
    if policy == None:
        ...
        if not len(policy) == board_height * board_width * board_height * board_width:
    ...
    while cat_pos != mouse_pos:
        if policy_type == 'random':
            ...
            cat_action_index = policy[state_index]
